...
class BatteryProbeClassification(nn.Module):
    ...
    def __init__(self, device, probe_class, num_task, input_dim=768, ce_weights = None):  # from 0 to 15
        super().__init__()
...
class BatteryProbeClassificationTwoLayer(nn.Module):
    # combines 64 classification problem for the case of Othello
    def __init__(self, device, probe_class, num_task, mid_dim,  input_dim=768, ce_weights = None):  # from 0 to 15
        super().__init__()
        self.input_dim = input_dim
        self.probe_class = probe_class
        self.num_task = num_task
...
class BatteryProbeClassificationIndependent(nn.Module):
    # combines 64 classification problem for the case of Othello
    ...
    def __init__(self, device, probe_class, num_task, input_dim=768, ce_weights = None):  # from 0 to 15
        ...
        self.proj = nn.ModuleList(
            [nn.Linear(self.input_dim, self.probe_class, bias=True) for _ in range(num_task)]
        )
